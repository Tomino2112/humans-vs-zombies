/*
  Java Application - Human vs. Zombies
  Author: Tomas Dostal  
  ---
  This script creates a grid with Humans and zombies and simulates the "life-cycle".
  
  Interactions between humans and zombies:
  Human vs. Human - nothing
  Human vs. Zombie - The one with greater strength wins. 
    If human loses, zombie gets more strength and human changes into zombie.
    If human wins, zombie disappears and human gets more strength
  
  Battles are only 1 on 1. Means if zombie is surrounded by 4 human the fight will be individual.
  Creatures are not getting tired for now. Means if Zombie with strength 10 is surrouded by few humans with strength 8 it will kill all of them.
  
  @todo: Make creatures loose and gain their strength continuously. (loosig whle fighting, gaining while not fighting, gainint more while winning), when strength reaches 0 they are dead (should be only for human)         
*/
...
class HumanVsZombie {
    public static void main(String[] args){
        ...
        int gridW, gridH, humanNum, zombieNum, posX, posY;              
        
        // Print out introduction
        System.out.println("==========================================");
        System.out.println("  Welcome to Human vs. Zombie Simulator  ");
        System.out.println("==========================================");
        System.out.println();
        ...
        // Create GUI - Upcoming
        /*JFrame frame = new JFrame("Humans vs. Zombies");    
        ...
        System.out.print("Number of zombies (0-"+(grid.getArea()-humanNum)+"): ");
        zombieNum = in.nextInt();      
        if (zombieNum<0 || zombieNum>grid.getArea()-humanNum){
            zombieNum = grid.getArea()/3;   // In case input is wrong, set one third as default;
        }  
        ...
        for(int i=0;i<zombieNum;i++){
            do {            
                posX = Helpers.randomNum(0,grid.getWidth()-1);
                posY = Helpers.randomNum(0,grid.getHeight()-1);
            } while (!grid.addCreature(posX, posY, new Creature("zombie",Helpers.randomNum(1,10),1)));                 
        }
        
        System.out.println();
        System.out.print("You have just created Human vs Zombie grid size of "+grid.getSizeString()+" ");
        System.out.println("with "+humanNum+" humans and "+zombieNum+" Zombies!");
        System.out.println();
